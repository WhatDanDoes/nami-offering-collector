...
function dataSigner(payload, privateKey, baseAddress) {
   ...
   */
  const builder = COSESign1Builder.new(headers, CBORValue.new_bytes(Buffer.from(payload)).to_bytes(), false);
  ...
  return Buffer.from(cose_sign1.to_bytes(), 'hex').toString('hex');
};
